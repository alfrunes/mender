...
func LoadConfig(configFile string) (*menderConfig, error) {
	...
	if err := readConfigFile(&confFromFile, configFile); err != nil {
		...
		log.Infof("Error loading configuration from file: %s (%s)",
	...
	if confFromFile.Servers != nil {
		if confFromFile.ServerURL != "" || confFromFile.TenantToken != "" {
			log.Error("In mender.conf: don't specify both Servers" +
				" field AND the corresponding fields in base" +
				" structure (i.e. ServerURL etc.). The first" +
				" server on the list on the list overwrites" +
				" these fields.")
			return nil, errors.New("Both Servers AND ServerURL / " +
				"TenantToken given in mender.conf")
		}
